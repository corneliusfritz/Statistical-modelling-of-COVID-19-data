...
nowcasting <- function(doa, T_0, d_max, n = 100,
  ...
  data <- data %>%
    ...
    dplyr::filter(date %in% dates, delay <= d_max, delay > 0,
                  date_report <= doa)
  ...
  data_grid <- expand.grid(dates, c(dates + d_max, dates),
    ...
    distinct()
  colnames(data_grid) <- c("date", "date_report", "age_group")
...
add_older_data <- function(data_report, data_old, quantiles) {
  ...
  data_old <- data_old %>%
    ...
    tidyr::complete(date = seq.Date(min(date), max(date), by = "day"), age_group,
             fill = list(C_t_d = 0)) %>%
    distinct()
